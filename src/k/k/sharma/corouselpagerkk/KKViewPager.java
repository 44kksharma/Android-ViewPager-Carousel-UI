...
public class KKViewPager extends ViewPager {
	...
	public enum TransitionEffect {
		ToggleFade,	Standard, Tablet, CubeIn, CubeOut, ZoomOut, RotateUp, RotateDown
	}
	...
	public KKViewPager(Context context, AttributeSet attrs) {
		...
		TypedArray ta = context.obtainStyledAttributes(attrs,
				R.styleable.KKViewPager);
		int effect = ta.getInt(R.styleable.KKViewPager_style, 0);
		String[] transitions = getResources().getStringArray(
				R.array.kk_effects);
		setTransitionEffect(TransitionEffect.valueOf(transitions[effect]));
		setFadeEnabled(ta.getBoolean(R.styleable.KKViewPager_fadeEnabled,
				false));
		setOutlineEnabled(ta.getBoolean(
				R.styleable.KKViewPager_outlineEnabled, false));
		setOutlineColor(ta.getColor(R.styleable.KKViewPager_outlineColor,
				Color.WHITE));
		switch (mEffect) {
